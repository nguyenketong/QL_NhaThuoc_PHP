...
class BaiVietController extends AdminController
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            $stmt = $this->db->prepare("
                INSERT INTO BAIVIET (TieuDe, MoTaNgan, NoiDung, HinhAnh, NgayDang, LuotXem, IsNoiBat, IsActive)
                VALUES (?, ?, ?, ?, NOW(), 0, ?, 1)
            ");
            $stmt->execute([
                $_POST['TieuDe'] ?? '',
    ...
    public function edit($id)
    {
        ...
        if ($this->isPost()) {
            ...
            $stmt = $this->db->prepare("
                UPDATE BAIVIET SET TieuDe = ?, MoTaNgan = ?, NoiDung = ?, HinhAnh = ?, IsNoiBat = ?
                WHERE MaBaiViet = ?
            ");
            $stmt->execute([
                $_POST['TieuDe'] ?? '',
    ...
    {
        $stmt = $this->db->prepare("UPDATE BAIVIET SET IsNoiBat = NOT COALESCE(IsNoiBat, 0) WHERE MaBaiViet = ?");
        $stmt->execute([$id]);
        $this->redirect('?controller=bai-viet');
    }
    ...
    {
        $stmt = $this->db->prepare("UPDATE BAIVIET SET IsActive = NOT COALESCE(IsActive, 0) WHERE MaBaiViet = ?");
        $stmt->execute([$id]);
        $this->redirect('?controller=bai-viet');
    }
}
