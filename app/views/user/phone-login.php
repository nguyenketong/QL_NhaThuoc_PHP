<?php
/**
 * Đăng nhập bằng số điện thoại hoặc Google
 */
?>
<div class="container py-5">
    <div class="row justify-content-center">
        <div class="col-md-5">
            <div class="card shadow">
                <div class="card-header bg-primary text-white text-center">
                    <h4 class="mb-0"><i class="bi bi-box-arrow-in-right"></i> Đăng nhập</h4>
                </div>
                <div class="card-body p-4">
                    
                    <?php if (!empty($_SESSION['flash']['error'])): ?>
                        <div class="alert alert-danger py-2">
                            <?= $_SESSION['flash']['error']; unset($_SESSION['flash']['error']); ?>
                        </div>
                    <?php endif; ?>

                    <?php if (!empty($_SESSION['flash']['success'])): ?>
                        <div class="alert alert-success py-2">
                            <?= $_SESSION['flash']['success']; unset($_SESSION['flash']['success']); ?>
                        </div>
                    <?php endif; ?>

                    <!-- Đăng nhập bằng Google -->
                    <?php if (!empty($googleLoginUrl)): ?>
                        <a href="<?= $googleLoginUrl ?>" class="btn btn-outline-dark btn-lg w-100 mb-3">
                            <svg width="20" height="20" viewBox="0 0 24 24" class="me-2">
                                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                            </svg>
                            Đăng nhập bằng Google
                        </a>
                        
                        <div class="text-center my-3">
                            <span class="text-muted">hoặc</span>
                        </div>
                    <?php endif; ?>

                    <!-- Đăng nhập bằng SĐT -->
                    <p class="text-muted text-center mb-3">Nhập số điện thoại để nhận mã OTP</p>
                    
                    <form action="<?= BASE_URL ?>/user/sendOtp" method="POST">
                        <div class="mb-4">
                            <label class="form-label">Số điện thoại</label>
                            <div class="input-group">
                                <span class="input-group-text"><i class="bi bi-phone"></i></span>
                                <input type="tel" name="soDienThoai" class="form-control form-control-lg" 
                                       placeholder="0912345678" required pattern="[0-9]{10,11}">
                            </div>
                            <small class="text-muted">Nhập số điện thoại 10-11 số</small>
                        </div>

                        <button type="submit" class="btn btn-primary btn-lg w-100">
                            <i class="bi bi-send"></i> Gửi mã OTP
                        </button>
                    </form>

                    <hr class="my-4">

                    <div class="text-center">
                        <p class="text-muted mb-2">Cần hỗ trợ? Liên hệ hotline</p>
                        <a href="tel:<?= STORE_PHONE ?>" class="btn btn-outline-success">
                            <i class="bi bi-telephone"></i> <?= STORE_PHONE ?>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
